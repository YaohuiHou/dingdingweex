...
// // 打开的链接地址
// const openUrl = "http://192.168.1.171:8083/dist/"
...
// 返回要求接口地址
function getNeedUrl(type) {
    const action = type;
    let timestamp = timest();
    let url = gateway + action + "?Action=" + action + "&AppId=" + appid + "&SecretKey=" + secretkey + "&TimeStamp=" + timestamp;
    return url
}

// 获取 sign
function getNeedUrlsign(url) {
    let sign = encodeURIComponent(b64_hmac_sha1(secretkey, encodeURIComponent(url).toLowerCase()));
    return sign;
}
